...
function Floor() {
  return (
    <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -2, 0]} receiveShadow>
      ...
      <meshStandardMaterial color="#666666" metalness={0.3} roughness={0.1} />
    </mesh>
  );
}
...
function ServerRackModel() {
  ...
  // Load GLB model
  const gltf = useLoader(GLTFLoader, "/models/server.glb");
...
function ScreenModel() {
  ...
  // Position on top of server rack - highlighting the position here for adjustment
  // Y position: 8 places it on top of the server (server goes from -2 to ~6 with 5x scale)
  // X and Z: centered on the server
  return (
    <group
      ref={groupRef}
      scale={[5, 5, 5]}
